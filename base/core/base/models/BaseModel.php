...
abstract class BaseModel extends BaseModelMethods
{
     ...
     * @param array $set - array parameters:
    ...
    public function getUnion($set = []){
        ...
        $unionType = ' UNION ' . (!empty($set['unionType']) ? strtoupper($set['unionType']) . ' ' : '');
            ...
            if (!empty($item['join'])){
                ...
                foreach ($item['join'] as $table => $data){
                    if (array_key_exists('fields', $data) && $data['fields']){
                        ...
                        (!$joinFields['data'] || $data['fields'] === null)){
        ...
        if ($maxCount && $maxTableCount){
            $query .= '(' . $this->get($maxTableCount,
                    $this->union[$maxTableCount]) . ')';
