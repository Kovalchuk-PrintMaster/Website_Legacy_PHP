...
abstract class BaseModelMethods
{
    ...
    protected function createFields($set, $table = false, $join=false){
       ...
       if(($join || (isset($set['join_structure']) && $set['join_structure']))
           && $table){
           $join_structure = true;

           $this->showColumns($table);

           if(isset($this->$table[$table]['multi_id_row'])) $set['fields'] = [];
       }
               ...
               if($field || $field === null){
                   ...
                   if($join && $join_structure){
                       if(preg_match('/^(.+)?\s+as\s+(.+)/i', $field, $matches)){
                            ...
                            $fields .= $concat_table . $field . ' as TABLE' .
                                $alias_table . 'TABLE_' . $field . ', ';
                       }
                   }else{
                       $fields .= (!preg_match('/(\([^()]*\))|(case\s+.+?\s+end)/i', $field)
                               ? $concat_table : ' ') . $field . ',';
                   }
               }
           }
                if(!$id_field && $join_structure){
                    if($join){
                        $fields .= $concat_table . $this->tableRows[$alias_table]['id_row'] . ' as TABLE' .
                            $alias_table . 'TABLE_' . $this->tableRows[$alias_table]['id_row'] . ',';
