...
use core\user\controllers\SendMailController;
...
class OrdersController extends BaseUser
{
    ...
    protected function order()
    {
        ...
        if (empty($this->cart['goods']) || empty($_POST)) {
            ...
            $this->sendError('Ой, покишо нема що оформлювати :(');
        }
        ...
        $validation = [
            ...
            'delivery_id' => [
                'translate' => 'Доступні способи доставки',
                'methods' => ['emptyField', 'numericField']
            ],
            'payments_id' => [
                'translate' => 'Доступні варіанти оплати',
                'methods' => ['emptyField', 'numericField']
            ],
        ];
        ...
        if (empty($visitor['email']) && empty($visitor['phone'])){
            ...
            $this->sendError('Упс. Відсутні ваші данні для оформлення замаовлення');
        }
       ...
       // remove [0] because not send 'visitors_id' to 'orders' table video - 157
        ...
        if (!$order['id']){
            ...
            $this->sendError('Ой, здається ваше замовлення не збереглося, спробуйте ще раз, або затерефонуйте нам для продовження оформлення замовлення - ' . $this->set['phone']);
        }
        ...
        if (!($goods = $this->setOrdersGoods($order))){
            ...
            $this->sendError('Ой, здаеться ваше замовлення не збереглося спробуйте ще раз, або зверніться до менеджера для продовження оформлення');
        ...
        $this->sendSuccess('Дякуємо за замовлення. Вже починаємо опрацьовувати, якщо буде потрібно уточнити деталі - то ми вам зателефонуемо.');
    ...
    protected function setOrdersGoods(array $order) : ? array{
      ...
      if (in_array('orders_goods', $this->model->showTables())){
          ...
          foreach ($this->cart['goods'] as $key=>$item){
              ...
              $prepareGoods[$key] = $item;

              $prepareGoods[$key]['total_sum'] = $item['qty'] * $item['price'];
...
//                          if (!empty($this->model->showColumns('orders_goods')['goods_id'])){ //lesson 150
