                    ...
                    success: res => {
                        ...
                        try{
                            ...
                            ['data-totalQty', 'data-totalSum', 'data-totalOldSum'].forEach(attr=>{
                                ...
                                let cartAttr = attr.replace(/data-/, '').replace(/([^A-Z])([A-Z])/g, '$1_$2').toLocaleLowerCase()

                                document.querySelectorAll(`[${attr}]`).forEach(el => {

                                    if (typeof res[cartAttr] !== 'undefined'){

                                        el.innerHTML =res[cartAttr] + (attr === 'data-totalQty' ? '' : ' грн')
...
// lesson 146
    ...
    item.addEventListener('input', e =>{
        ...
        if (item.value){
            ...
            for (let code in countriesOption){

                if (countriesOption.hasOwnProperty(code)){

                    let regExp = new RegExp(`^[${countriesOption[code].firstDigits}]`)
            ...
            for (let code in countriesOption){
                ...
                if (countriesOption.hasOwnProperty(code) && countriesOption[code].firstDigits){

                    let regExp = new RegExp(code.replace(/\+/g, '\\+'), 'g')

                    if (regExp.test(item.value)){

                        for (let i in countriesOption[code].formatChars){

                            let j = +i


                            if (item.value[j] && item.value[j] !== countriesOption[code].formatChars[i]){

                                item.value = item.value.substring(0, j) + countriesOption[code].formatChars[i] + item.value.substring(j)
                            }
                        }

                        if (item.value[countriesOption[code].limit]){

                            item.value = item.value.substring(0, countriesOption[code].limit)
