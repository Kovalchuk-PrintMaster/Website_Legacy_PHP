# ForPrint frontend CSS ownership and layout strategy v0.2

**Status:** active working policy  
**Verified against repository loading path:** 2026-07-19  
...
## 2.1. Verified current CSS loading chain

The public frontend currently loads CSS through this sequence:

1. `base/core/base/settings/internal_settings.php` defines `USER_CSS_JS['styles']`.
2. `base/core/base/controllers/BaseController.php` copies those entries into `$this->styles`.
3. Page controllers may append page-specific styles to the same array.
4. `base/core/base/controllers/BaseMethods.php::getStyles()` renders the array in order.
5. `base/templates/default/include/header.php` currently adds several hardcoded modern CSS links after `getStyles()`.

Current public global registry order:
...
Current hardcoded links after `getStyles()`:
...
This means the safe first registration point for `forprint-layout.css` is directly after `style.css` inside `USER_CSS_JS['styles']`.

The resulting intended transitional order is:

```text
vendor CSS
legacy fallback
forprint-layout.css
controller-owned surface CSS
temporarily hardcoded modern component CSS
```

The hardcoded links are accepted as temporary technical debt. They must later move to one explicit registry or to their actual surface controllers.

## 2.2. Admin CSS boundary

`ADMIN_CSS_JS['styles']` uses the admin stylesheet `css/main.css`.

The admin `main.css` is not part of the public frontend migration described by this document. Public frontend modernization must not modify or remove the admin CSS registration unless a separate admin-interface task explicitly requires it.
...
- Verify the actual public CSS loading chain from registry to rendered `<link>` tags.
- Define `USER_CSS_JS['styles']` immediately after `style.css` as the transitional registration point for `forprint-layout.css`.
- Record hardcoded modern CSS links in `include/header.php` as temporary technical debt.
- Separate the admin `css/main.css` boundary from the public frontend migration.
